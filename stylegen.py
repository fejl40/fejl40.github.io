...
def stylespaste(list):
    for i in list:
        ...
        .{filename} {forwardBracket}
            width: 1280px;
            height: 960px;
            background: url("../maps/vertigo/{filename}.jpg") no-repeat;
            display: inline-block;
        {BackBracket}
        .{filename}:hover {forwardBracket}
            background: url("../maps/vertigo/{filename}_2.jpg") no-repeat;
        {BackBracket}
...
def scrollList(list):
    ...
    print("</ul>")
def main():
    files = os.listdir("/Users/krois/Downloads/maps/vertigo")
